...
def save_manifest(docs_dir: Path, manifest: dict) -> None:
    ...
    manifest["base_url"] = (
        f"https://raw.githubusercontent.com/{github_repo}/{github_ref}/docs/claude-code/"
    )
    manifest["github_repository"] = github_repo
    manifest["github_ref"] = github_ref
    manifest["description"] = (
        "Claude Code documentation manifest. Keys are filenames, append to base_url for full URL."
    )
...
def validate_markdown_content(content: str, filename: str) -> None:
    ...
    if indicator_count < 3:
        raise ValueError(
            f"Content doesn't appear to be markdown "
            f"(only {indicator_count} markdown indicators found)"
        )
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            header = (
                ...
                "> \n"
                "> This is the official Claude Code release changelog, automatically "
                "fetched from the Claude Code repository. For documentation, see other "
                "topics via `/docs`.\n"
                "\n"
                "---\n"
                "\n"
...
def main():
    ...
    try:
        ...
        new_manifest["files"][filename] = {
            ...
            "original_raw_url": original_raw_url,
            "hash": content_hash,
            "last_updated": last_updated,
