...
Cursor loads `~/.cursor/skills/` and `~/.agents/skills/` on the local machine. Those folders are not copied to Cloud Agents, Agents Window remote SSH, or [BYOM workers](https://cursor.com/docs/cloud-agent/bring-your-own-machine/pools.md). On those surfaces, use project skills in the repo, or bake skills into the worker image.
