...
- **Remote Control.** Hand off an agent you're running on your computer, then keep directing it from your phone. Turn on Remote Control in **Settings > Agents**, run `/remote-control` in the agent's input, and open the session in the app. The agent loop moves to the cloud while its tools keep running on your machine. Remote Control is in beta and needs a paid plan.
