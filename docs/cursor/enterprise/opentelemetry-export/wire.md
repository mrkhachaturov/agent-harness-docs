...
| `cursor.user.email`      | string | Optional | The member's email. Present only on teams where Cursor has enabled email export, and then only alongside `cursor.user.id`; ask your account team. Do not require presence.                                                                        |
...
The `grok_bot.*` events carry Bot actions from Action Recording. Family `grok_bot_agent_actions`. Every record has `cursor.surface=grok_bot`. `cursor.conversation.id` is the identifier for the Bot, and that value is the Bot's conversation id.
...
| Group by user                              | Resource attribute `cursor.user.account_id` | Logs and metrics when present. Joins to `id` in the Admin API `GET /teams/members` response. `cursor.user.email`, present only on teams where Cursor has enabled it, names the member directly. |
